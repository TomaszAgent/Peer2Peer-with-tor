...
def start_chat():
    ...
    choosen_user = users[click.prompt("Type nickname", type=str)]

    with socks.socksocket() as socket_connection:
        click.echo(f"Connecting to {choosen_user[0]} on port {choosen_user[1]}...")
        socket_connection.connect((f"{choosen_user[0]}.onion", int(choosen_user[1])))

        socket_connection.sendall(f"{nick} wants to chat with you\n".encode())

        chats[choosen_user] = {
            "messages": [],
            "socket": socket_connection,
            "address": choosen_user
        }
